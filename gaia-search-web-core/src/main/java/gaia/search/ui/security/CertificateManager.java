 ...
 */
package gaia.search.ui.security;

import gaia.search.ui.configuration.Configuration;
import gaia.search.ui.utils.ShellCommandUtil;
...
/**
 * Gaia security. Manages server and agent certificates
 */
@Singleton
public class CertificateManager {
	...
	private static final String SIGN_SRVR_CRT = "openssl x509 "
			+ "-passin pass:{0} -req -days 365 -in {1}/{3} -signkey {1}/{2} " + "-out {1}/{3} \n";
	private static final String EXPRT_KSTR = "openssl pkcs12 -export"
			+ " -in {1}/{3} -inkey {1}/{2} -certfile {1}/{3} -out {1}/{4} " + "-password pass:{0} -passin pass:{0} \n";
	...
	private void generateServerCertificate() {
		...
		Object[] scriptArgs = { srvrCrtPass, srvrKstrDir, srvrKeyName, srvrCrtName, kstrName };
