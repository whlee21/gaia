...
public class Users {
	...
	@Inject
	private GaiaLdapAuthenticationProvider ldapAuthenticationProvider;
	...
	public User getUser(int userId) throws GaiaSearchUIServerException {
		UserEntity userEntity = userDAO.findByPK(userId);
		if (userEntity != null) {
			return new User(userEntity);
		} else {
			throw new GaiaSearchUIServerException("User with id '" + userId + " not found");
		}
	}
	...
	public User getLocalUser(String userName) throws GaiaSearchUIServerException {
		UserEntity userEntity = userDAO.findLocalUserByName(userName);
		if (userEntity == null) {
			throw new GaiaSearchUIServerException("User doesn't exist");
		}
		return new User(userEntity);
	}

	public User getLdapUser(String userName) throws GaiaSearchUIServerException {
		UserEntity userEntity = userDAO.findLdapUserByName(userName);
		if (userEntity == null) {
			throw new GaiaSearchUIServerException("User doesn't exist");
		}
	 ...
	 * 
	 * @throws GaiaSearchUIServerException
	 */
	public synchronized void modifyPassword(String userName, String currentUserPassword, String newPassword)
		...
		if (currentUserName == null) {
			throw new GaiaSearchUIServerException("Authentication required. Please sign in.");
		}
		...
		if (currentUserEntity == null) {
			...
			try {
				ldapAuthenticationProvider.authenticate(new UsernamePasswordAuthenticationToken(currentUserName,
						currentUserPassword));
				isLdapAdmin = true;
			} catch (BadCredentialsException ex) {
				throw new GaiaSearchUIServerException("Incorrect password provided for LDAP user " + currentUserName);
			}
		}
		...
		if ((userEntity != null) && (currentUserEntity != null)) {
			if (isLdapAdmin || passwordEncoder.matches(currentUserPassword, currentUserEntity.getUserPassword())) {
				...
				throw new GaiaSearchUIServerException("Wrong password provided");
			}

		} else {
			userEntity = userDAO.findLdapUserByName(userName);
			if (userEntity != null) {
				throw new GaiaSearchUIServerException("Password of LDAP user cannot be modified");
			} else {
				throw new GaiaSearchUIServerException("User " + userName + " not found");
			}
		}
	}
	...
	@Transactional
	public synchronized void removeUser(User user) throws GaiaSearchUIServerException {
		UserEntity userEntity = userDAO.findByPK(user.getUserId());
		if (userEntity != null) {
			userDAO.remove(userEntity);
		} else {
			throw new GaiaSearchUIServerException("User " + user + " doesn't exist");
		}
	}
	 ...
	 * @throws GaiaSearchUIServerException
	 */
	public synchronized void promoteToAdmin(User user) throws GaiaSearchUIServerException {
	 ...
	 * @throws GaiaSearchUIServerException
	 */
	public synchronized void demoteAdmin(User user) throws GaiaSearchUIServerException {
		removeRoleFromUser(user, getAdminRole());
	}

	@Transactional
	public synchronized void addRoleToUser(User user, String role) throws GaiaSearchUIServerException {

		if (userDAO.findLdapUserByName(user.getUserName()) != null) {
			LOG.warn("Trying to add a role to the LDAP user" + ", user=" + user.getUserName());
			throw new GaiaSearchUIServerException("Roles are not editable for LDAP users");
		}
		...
		if (roleEntity == null) {
			...
			throw new GaiaSearchUIServerException("Role " + role + " doesn't exist");
		}
		...
		if (!userEntity.getRoleEntities().contains(roleEntity)) {
			...
			throw new GaiaSearchUIServerException("User " + user + " already owns role " + role);
		}

	}

	@Transactional
	public synchronized void removeRoleFromUser(User user, String role) throws GaiaSearchUIServerException {
		...
		if (userEntity.getRoleEntities().contains(roleEntity)) {
			...
			throw new GaiaSearchUIServerException("User " + user + " doesn't own role " + role);
		}
