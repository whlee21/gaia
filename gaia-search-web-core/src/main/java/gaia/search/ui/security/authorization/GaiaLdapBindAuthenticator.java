 ...
 */
public class GaiaLdapBindAuthenticator extends BindAuthenticator {

	private Configuration configuration;

	private static final String GAIA_ADMIN_LDAP_ATTRIBUTE_KEY = "gaia_admin";

	public GaiaLdapBindAuthenticator(BaseLdapPathContextSource contextSource, Configuration configuration) {
		super(contextSource);
	...
	private DirContextOperations setAmbariAdminAttr(DirContextOperations user) {
		...
		List<String> gaiaAdminGroups = ldapTemplate.search(groupBase, filterBuilder.toString(), attributesMapper);

		// user has admin role granted, if user is a member of at least 1 group,
		// which matches the rules in configuration
		if (gaiaAdminGroups.size() > 0) {
			user.setAttributeValue(GAIA_ADMIN_LDAP_ATTRIBUTE_KEY, true);
		}
