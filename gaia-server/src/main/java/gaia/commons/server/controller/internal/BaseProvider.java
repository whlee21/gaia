...
public abstract class BaseProvider {
	 ...
	 * The logger.
	 */
	protected final static Logger LOG = LoggerFactory.getLogger(BaseProvider.class);
	 ...
	 *          the config category that would have a <code>desired_config</code>
	 *          element.
	...
	public Set<String> checkPropertyIds(Set<String> propertyIds) {
		if (!this.propertyIds.containsAll(propertyIds)) {
			LOG.debug("whlee21 propertyIds = " + propertyIds);
			Set<String> unsupportedPropertyIds = new HashSet<String>(propertyIds);
			LOG.debug("whlee21 combinedIds = " + combinedIds);
			...
			// properties where
			// we want to treat property as a category and the entries as
	...
	protected Set<String> getRequestPropertyIds(Request request, Predicate predicate) {
		...
		if (!this.combinedIds.containsAll(propertyIds)) {
			...
			// Add the categories to account for map properties where the
			// entries will not be
			// in the provider property list ids but the map (category) might
			// be.
			for (String unsupportedPropertyId : unsupportedPropertyIds) {
				String category = PropertyHelper.getPropertyCategory(unsupportedPropertyId);
	...
	/**
	 * Set a property value on the given resource for the given id and value. Make
	 * sure that the id is in the given set of requested ids.
	 * 
	 * @param resource
	 *          the resource
	 * @param propertyId
	 ...
	 *          the value to set
	 * @param requestedIds
	 *          the requested set of property ids
	 */
	protected static boolean setResourceProperty(Resource resource, String propertyId, Object value,
		...
		if (contains) {
			if (LOG.isDebugEnabled()) {
				LOG.debug("Setting property for resource" + ", resourceType=" + resource.getType() + ", propertyId="
			...
			if (value instanceof Map<?, ?>) {
				// This map wasn't requested, but maybe some of its entries
				// were...
			...
			if (!contains && LOG.isDebugEnabled()) {
				LOG.debug("Skipping property for resource as not in requestedIds" + ", resourceType=" + resource.getType()
	...
	/**
	 * If the given value is a Map then add its entries to the resource as
	 * properties.
	 * 
	 * @param resource
	 *          the resource
	 * @param propertyId
	 *          the property id of the given value
	 * @param value
	 *          the property value
	 */
		...
		if (value instanceof Map<?, ?>) {
			...
			if (mapValue.isEmpty()) {
				...
				for (Map.Entry<?, ?> entry : mapValue.entrySet()) {
					...
					// If the value is a Map then set all of its entries as
					// properties
