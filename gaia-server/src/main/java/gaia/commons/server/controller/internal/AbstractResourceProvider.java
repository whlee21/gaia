...
import gaia.commons.server.DuplicateResourceException;
import gaia.commons.server.GaiaException;
import gaia.commons.server.ObjectNotFoundException;
import gaia.commons.server.ParentObjectNotFoundException;
 ...
 * controller.
 */
public abstract class AbstractResourceProvider extends BaseProvider implements ResourceProvider,
	...
	/**
	 * Get the set of property ids that uniquely identify the resources of this
	 * provider.
	 * 
	 ...
	 * 
	 * @param type
	 *          the type of the resources being created
	 * @param request
	 *          the request used to create the resources
	 */
	 ...
	 * @param predicate
	 *          the predicate used to update the resources
	 */
	 ...
	 * 
	 * @param type
	 *          the type of the resources being deleted
	 * @param predicate
	 ...
	 * property/value mappings is required to generate delete or get requests to
	 * the back end which does not deal with predicates. Note that the single
	 * predicate can result in multiple backend requests.
	 * 
	 * @param givenPredicate
	 *          the predicate
	 * 
	 * @return the set of properties used to build request objects
	 */
	protected Set<Map<String, Object>> getPropertyMaps(Predicate givenPredicate) throws UnsupportedPropertyException,
			SystemException, NoSuchResourceException, NoSuchParentResourceException {

		LOG.debug("getPropertyMaps 11");
	 ...
	 * returned set of property/value mappings is required to generate update or
	 * create requests to the back end which does not deal with predicates. Note
	 * that the single property map & predicate can result in multiple backend
	 * requests.
	 * 
	 * @param requestPropertyMap
	 *          the request properties (for update)
	 * @param givenPredicate
	 *          the predicate
	 * 
	 * @return the set of properties used to build request objects
	...
	/**
	 * Invoke a command against the Ambari backend to create resources and map any
	 * {@link AmbariException} to the types appropriate for the
	 ...
	 *           thrown if a system exception occurred
	 * @throws ResourceAlreadyExistsException
	 *           thrown if a resource already exists
	 * @throws NoSuchParentResourceException
	 *           thrown if a parent of a resource doesn't exist
	 */
	protected <T> T createResources(Command<T> command) throws SystemException, ResourceAlreadyExistsException,
			NoSuchParentResourceException {
		try {
			return command.invoke();
		} catch (ParentObjectNotFoundException e) {
			throw new NoSuchParentResourceException(e.getErrors(), e);
		} catch (DuplicateResourceException e) {
			throw new ResourceAlreadyExistsException(e.getErrors());
		} catch (GaiaException e) {
			if (LOG.isErrorEnabled()) {
				LOG.error("Caught GaiaException when creating a resource", e);
			}
			throw new SystemException(e.getStatus(), e.getErrors(), e);
		}
	}

	/**
	 * Invoke a command against the Ambari backend to get resources and map any
	 * {@link AmbariException} to the types appropriate for the
	 * {@link ResourceProvider} interface.
	 * 
	 * @param command
	 ...
	 *           thrown if a parent of a resource doesn't exist
	 */
	protected <T> T getResources(Command<T> command) throws SystemException, NoSuchResourceException,
			NoSuchParentResourceException {
		try {
			return command.invoke();
		} catch (ObjectNotFoundException e) {
			throw new NoSuchResourceException(e.getErrors(), e);
		} catch (ParentObjectNotFoundException e) {
			throw new NoSuchParentResourceException(e.getErrors(), e);
		} catch (GaiaException e) {
			if (LOG.isErrorEnabled()) {
				LOG.error("Caught GaiaException when getting a resource", e);
			}
			throw new SystemException(e.getStatus(), e.getErrors(), e);
		}
	}
	...
	/**
	 * Invoke a command against the Ambari backend to modify resources and map any
	 * {@link AmbariException} to the types appropriate for the
	 * {@link ResourceProvider} interface.
	 * 
	 * @param command
	 ...
	 *           thrown if a parent of a resource doesn't exist
	 */
	protected <T> T modifyResources(Command<T> command) throws SystemException, NoSuchResourceException,
			NoSuchParentResourceException {
		try {
			return command.invoke();
		} catch (ObjectNotFoundException e) {
			...
			if (LOG.isErrorEnabled()) {
				LOG.error("Caught GaiaException when modifying a resource", e);
			}
			throw new SystemException(e.getStatus(), e.getErrors(), e);
		}
	}
	 ...
	 *          the parent category name. Checks for a property whose category is
	 *          the parent and marked as a desired config.
	 * @param properties
	 *          the properties on the request.
	 */
	// protected ConfigurationRequest getConfigurationRequest(
	// String parentCategory, Map<String, Object> properties) {
	//
	// ConfigurationRequest config = null;
	//
	// // as a convenience, allow consumers to specify name/value overrides in
	// // this
	// // call instead of forcing a cluster call to do that work
	// for (Entry<String, Object> entry : properties.entrySet()) {
	// String absCategory = PropertyHelper.getPropertyCategory(entry
	// .getKey());
	...
	// config.setSelected(Boolean.parseBoolean(entry.getValue()
	// .toString()));
	// } else if (absCategory.endsWith("/properties")) {
	// config.getProperties().put(propName,
	...
	protected interface Command<T> {
		 ...
		 *           thrown if a problem occurred during invocation
		 */
		public T invoke() throws GaiaException;
	}
}
