...
import gaia.commons.server.controller.predicate.OrPredicate;
 ...
 * )). This is done because an individual back end request object can not handle
 * OR. Instead we need to break up the predicate to form multiple requests and
 * take the union of all the responses. 2) convert predicates based on
 * unsupported properties to AlwaysPredicate. Unsupported properties are those
 * not returned by the resource provider. For these properties we need to wait
 * until the property provider that handles the property has been called. 3)
 * convert predicates based on any operator other than == to AlwaysPredicate.
 * The back end requests can not handle any operator other than equals. The
 * complete predicate will get applied further down the line if necessary.
 * 
 * After visiting a predicate, the visitor should be able to supply a list of
 * predicates that can be used to generate requests to the backend, working
 * around the restrictions above.
 * 
 * Note that the results acquired using the generated predicates may be a super
 * set of what is actually desired given the original predicate, but the
 * original predicate will be applied at a suitable time down the line if
 * required.
 */
public class SimplifyingPredicateVisitor implements PredicateVisitor {
	...
	public void acceptComparisonPredicate(ComparisonPredicate predicate) {
		if (predicate instanceof EqualsPredicate
				&& resourceProvider.checkPropertyIds(
						Collections.singleton(predicate.getPropertyId()))
						.isEmpty()) {
		...
		if (hasOrs && arrayPredicate instanceof AndPredicate) {
			...
			for (int i = 0; i < size; ++i) {
				for (int j = i + 1; j < size; ++j) {
					andPredicateList.addAll(distribute(predicateList.get(i),
							predicateList.get(j)));
				}
			}
			lastVisited = OrPredicate.instance(andPredicateList
					.toArray(new Predicate[andPredicateList.size()]));
		} else {
			lastVisited = arrayPredicate.create(predicateList
	...
	private static List<Predicate> distribute(Predicate left, Predicate right) {
		...
		return Collections.singletonList(left.equals(right) ? left
				: AndPredicate.instance(left, right));
	}
	...
	private static List<Predicate> distributeOr(OrPredicate orPredicate,
		...
		for (Predicate basePredicate : orPredicate.getPredicates()) {
			...
			if (otherOr != null) {
				...
				andPredicateList
						.add(basePredicate.equals(other) ? basePredicate
								: AndPredicate.instance(basePredicate, other));
			}
		}
