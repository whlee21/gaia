...
public abstract class ArrayPredicate implements BasePredicate {
	...
	public boolean equals(Object o) {
		...
		if (propertyIds != null ? !propertyIds.equals(that.propertyIds)
				: that.propertyIds != null)
			return false;

		// don't care about array order
		Set<Predicate> setThisPredicates = new HashSet<Predicate>(
				Arrays.asList(predicates));
		Set<Predicate> setThatPredicates = new HashSet<Predicate>(
				Arrays.asList(that.predicates));
	...
	public int hashCode() {
		// don't care about array order
		int result = predicates != null ? new HashSet<Predicate>(
				Arrays.asList(predicates)).hashCode() : 0;
		result = 31 * result
