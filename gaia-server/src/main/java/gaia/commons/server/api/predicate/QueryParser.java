...
import gaia.commons.server.api.predicate.expressions.Expression;
import gaia.commons.server.api.predicate.expressions.LogicalExpressionFactory;
import gaia.commons.server.api.predicate.expressions.RelationalExpression;
import gaia.commons.server.api.predicate.operators.LogicalOperator;
import gaia.commons.server.api.predicate.operators.LogicalOperatorFactory;
...
import gaia.commons.server.api.predicate.operators.RelationalOperatorFactory;
...
/**
 * Parser which produces a predicate instance from an array of tokens, which is
...
public class QueryParser {
	 ...
	 */
	public QueryParser() {
		TOKEN_HANDLERS.put(Token.TYPE.BRACKET_OPEN,
				new BracketOpenTokenHandler());
		TOKEN_HANDLERS.put(Token.TYPE.BRACKET_CLOSE,
				new BracketCloseTokenHandler());
		TOKEN_HANDLERS.put(Token.TYPE.RELATIONAL_OPERATOR,
				new RelationalOperatorTokenHandler());
		TOKEN_HANDLERS.put(Token.TYPE.LOGICAL_OPERATOR,
				new LogicalOperatorTokenHandler());
		TOKEN_HANDLERS.put(Token.TYPE.LOGICAL_UNARY_OPERATOR,
				new LogicalUnaryOperatorTokenHandler());
		TOKEN_HANDLERS.put(Token.TYPE.PROPERTY_OPERAND,
				new PropertyOperandTokenHandler());
		TOKEN_HANDLERS.put(Token.TYPE.VALUE_OPERAND,
				new ValueOperandTokenHandler());
		TOKEN_HANDLERS.put(Token.TYPE.RELATIONAL_OPERATOR_FUNC,
				new RelationalOperatorFuncTokenHandler());
	}

	/**
	 * Generate a Predicate instance from an array of tokens. Each input token
	 * contains a type and a value.
	 * 
	 * Based on the token type and location, the tokens are first translated
	 * into a list of expressions, both relational and logical. These
	 * expressions are then merged into a tree of expressions with a single root
	 * following operator precedence and explicit grouping rules. Depending on
	 * the query, this merging of expressions into a tree of expressions may
	 * occur in several passes, one pass per level of precedence starting at the
	 * highest level of precedence.
	 * 
	 * The predicate is built by traversing the expression tree in-order with
	 * each node expressing itself as a predicate.
	 * 
	 * @param tokens
	 *            an array of tokens which represent the query, each token
	...
	public Predicate parse(Token[] tokens) throws InvalidQueryException {
		...
		List<Expression> listMergedExpressions = mergeExpressions(
				listExpressions, ctx.getMaxPrecedence());

		return listMergedExpressions.isEmpty() ? null : listMergedExpressions
				.get(0).toPredicate();
	}

	/**
	 * Create parse context from an array of tokens. The parse context contains
	 * a list of expressions and other information about the expressions an
	 * parsed tokens.
	 * 
	 * @param tokens
	 *            an array of tokens which represent the query, each token
	 ...
	 * @throws InvalidQueryException
	 *             if unable to properly parse the tokens into a parse context
	 */
	private ParseContext parseExpressions(Token[] tokens)
			throws InvalidQueryException {
		ParseContext ctx = new ParseContext(tokens);

		while (ctx.getCurrentTokensIndex() < tokens.length) {
			TOKEN_HANDLERS.get(tokens[ctx.getCurrentTokensIndex()].getType())
					.handleToken(ctx);
		}

		if (ctx.getPrecedenceLevel() != 0) {
			throw new InvalidQueryException(
					"Invalid query string: mismatched parentheses.");
		}
	 ...
	 * This is done recursively in several passes, one per level of precedence
	 * starting at the highest precedence level. Recursion exits when a single
	 * expression remains.
	 * 
	 * @param listExpressions
	 *            list of expressions to merge
	 * @param precedenceLevel
	 *            the precedence level that is to be merged
	 * 
	...
	/**
	 * A parse context which contains information related to parsing the
	 * provided tokens into expressions.
	 */
	private class ParseContext {
		/**
		 * The current context precedence level. This is dictated by bracket
		 * tokens.
		 */
		...
		/**
		 * Set the current position in the tokens array. Each handler should set
		 * this value after processing a token(s).
		 * 
		 * @param idx
		 ...
		 * 
		 * @param val
		 *            how much the level is increased by
		 */
		 ...
		 * 
		 * @param val
		 *            how much the level is decremented by
		 * @throws InvalidQueryException
		 *             if the level is decremented below 0
		 */
		...
		public Expression getPrecedingExpression() {
			return m_listExpressions == null ? null : m_listExpressions
					.get(m_listExpressions.size() - 1);
		}

		/**
		 * Get the highest operator precedence in the list of generated
		 * expressions.
		 * 
		...
		/**
		 * Update the max precedence level. The max precedence level is only
		 * updated if the provided level > the current level.
		 * 
		 * @param precedenceLevel
		 *            the new value
		 */
	...
	/**
	 * Base token handler. Token handlers are responsible for handling the
	 * processing of a specific token type.
	 */
	private abstract class TokenHandler {
		/**
		 * Process a token. Handles common token processing functionality then
		 * delegates to the individual concrete handlers.
		 * 
		 * @param ctx
		 *            the current parse context
		 * @throws InvalidQueryException
		 *             if unable to process the token
		 */
			...
			if (!validate(ctx.getPreviousTokenType())) {
				throw new InvalidQueryException(
						"Unexpected token encountered in query string. Last Token Type="
								...
								+ ", Current Token[type=" + token.getType()
								+ ", value='" + token.getValue() + "']");
			}
			ctx.setTokenType(token.getType());

			int idxIncrement = _handleToken(ctx);
			ctx.setCurrentTokensIndex(ctx.getCurrentTokensIndex()
					+ idxIncrement);
		}
		 ...
		 *             if unable to process the token
		 */
		public abstract int _handleToken(ParseContext ctx)
				throws InvalidQueryException;
	...
	private class RelationalOperatorTokenHandler extends TokenHandler {
		...
		public int _handleToken(ParseContext ctx) throws InvalidQueryException {
			Token token = ctx.getTokens()[ctx.getCurrentTokensIndex()];
			RelationalOperator relationalOp = RelationalOperatorFactory
					.createOperator(token.getValue());
	...
	private class RelationalOperatorFuncTokenHandler extends TokenHandler {
		...
		public int _handleToken(ParseContext ctx) throws InvalidQueryException {
			...
			if (idx >= tokens.length
					|| tokens[idx].getType() != Token.TYPE.BRACKET_CLOSE) {
				throw new InvalidQueryException(
						"Missing closing bracket for in expression.");
			}
			return 1;
		}
	...
	private class LogicalOperatorTokenHandler extends TokenHandler {
		...
		public int _handleToken(ParseContext ctx) throws InvalidQueryException {
			Token token = ctx.getTokens()[ctx.getCurrentTokensIndex()];
			LogicalOperator logicalOp = LogicalOperatorFactory.createOperator(
					token.getValue(), ctx.getPrecedenceLevel());
			ctx.updateMaxPrecedence(logicalOp.getPrecedence());
			ctx.addExpression(LogicalExpressionFactory
					.createLogicalExpression(logicalOp));
	 ...
	 */
	private class LogicalUnaryOperatorTokenHandler extends
			LogicalOperatorTokenHandler {
		@Override
		public boolean validate(Token.TYPE previousTokenType) {
			return previousTokenType == null
					|| previousTokenType == Token.TYPE.BRACKET_OPEN
