...
public class ResultPostProcessorImpl implements ResultPostProcessor {
	private static final Logger LOG = LoggerFactory
			.getLogger(ResultPostProcessorImpl.class);
	/**
	...
	/**
	 * Map of resource post processors keyed by resource type. These are used to
	 * act on specific resource types contained in the result.
	 */
	...
	private void processNode(TreeNode<Resource> node, String href) {
		...
		if (r != null) {
			List<ResourceDefinition.PostProcessor> listProcessors = m_mapPostProcessors
					.get(r.getType());
	...
	/**
	 * Registers the resource processors. Recursively registers child resource
	 * processors.
	 * 
	 * @param resource
	 *            the root resource
	...
	private void registerResourceProcessors(ResourceInstance resource) {
		...
		for (ResourceInstance child : resource.getSubResources().values()) {
			// avoid cycle
			if (!m_mapPostProcessors.containsKey(child.getResourceDefinition()
					.getType())) {
				registerResourceProcessors(child);
			}
		}

		// always add Request post processors since they may be returned but
		// will not be a child
		// commented by whlee21
		// m_mapPostProcessors.put(Resource.Type.Request,
		// new RequestResourceDefinition().getPostProcessors());
	}
