...
public class DocumentTable extends HBaseTable {
	...
	public static final byte[] INFO_CONTENT_TYPE_CQ = Bytes
			.toBytes("contentType");
	...
	public void putDocument(Document doc, HTableInterface table)
			throws IOException {
		putDocument(doc, table, false);
	}
	...
	public void putDocument(Document doc, HTableInterface table,
		...
		if (!overwrite) {
			boolean ok;
			if (doc.version == 0L)
				ok = table.checkAndPut(put.getRow(), INFO_CF, INFO_VERSION_CQ,
						null, put);
			else {
				ok = table.checkAndPut(put.getRow(), INFO_CF, INFO_VERSION_CQ,
						Bytes.toBytes(doc.version), put);
			}
			if (!ok)
				throw new ConcurrentModificationException(
						"Version mismatch in Document record - HBase was not updated for doc: "
								+ doc.id + " with version: " + doc.version);
		} else {
			table.put(put);
		}
	}
	...
	public Document getDocument(String id, String collection, int mask,
			HTableInterface table) throws IOException {
	...
	public static Put newPut(Document doc) throws IOException {
		Put put = new Put(keySerializer.toBytes(new DocumentKey(doc.id,
				doc.collection)));
		...
		if (doc.contentType != null) {
			put.add(INFO_CF, INFO_CONTENT_TYPE_CQ,
					Bytes.toBytes(doc.contentType));
		}

		if (doc.boost != null) {
			put.add(INFO_CF, INFO_BOOST_CQ,
					Bytes.toBytes(doc.boost.doubleValue()));
		}
		...
		if (doc.fields != null) {
			SuffixKeyValueSerializer.KeyValue kv = serializer.toBytes("fields",
					doc.fields);
		...
		if (doc.calculated != null) {
			for (Map.Entry calc : doc.calculated.entrySet()) {
				SuffixKeyValueSerializer.KeyValue kv = serializer.toBytes(
						(String) calc.getKey(), calc.getValue());
	...
	public static Get newGet(String id, String collection, int mask)
			throws IOException {
		Get get = new Get(
				keySerializer.toBytes(new DocumentKey(id, collection)));
	...
	public static Delete newDelete(String id, String collection)
			throws IOException {
		Delete delete = new Delete(keySerializer.toBytes(new DocumentKey(id,
				collection)));
		return delete;
	}
	...
	public static Document resultToDocument(Result result) throws IOException {
		...
		Document doc = new Document(Bytes.toString(result.getValue(INFO_CF,
				INFO_ID_CQ)), Bytes.toString(result.getValue(INFO_CF,
				INFO_COLLECTION_CQ)), Bytes.toLong(result.getValue(INFO_CF,
				INFO_VERSION_CQ)));
		...
		if (result.containsColumn(INFO_CF, INFO_CONTENT_TYPE_CQ)) {
			doc.contentType = Bytes.toString(result.getValue(INFO_CF,
					INFO_CONTENT_TYPE_CQ));
		}

		if (result.containsColumn(INFO_CF, INFO_BOOST_CQ)) {
			doc.boost = Double.valueOf(Bytes.toDouble(result.getValue(INFO_CF,
					INFO_BOOST_CQ)));
		}

		if (result.containsColumn(TEXT_CF, TEXT_CONTENT_CQ)) {
			doc.text = Bytes
					.toString(result.getValue(TEXT_CF, TEXT_CONTENT_CQ));
		}
		...
		if (fieldMap != null) {
			for (Map.Entry<byte[], byte[]> column : fieldMap.entrySet()) {
				SuffixKeyValueSerializer.KeyValue<String, Object> kv = serializer.toObject(
						(byte[]) column.getKey(), (byte[]) column.getValue());
	...
	public Map<String, Double> listSimilarDocuments(String id, String collection)
			throws IOException {
		...
		if (doc != null) {
			...
			if (unsortedMap != null) {
				Comparator valueComparator = Ordering.natural().reverse()
						.onResultOf(Functions.forMap(unsortedMap))
						.compound(Ordering.natural());

				similarDocIdsWithScore = ImmutableSortedMap.copyOf(unsortedMap,
						valueComparator);
			}
		}
	...
	public void setSimilarDocuments(Document doc,
			Map<String, Double> similarDocIdsWithScore) {
	...
	protected byte[][] getSplits() {
		log.info("DocumentTable has "
				+ DocumentKeySerializer.getNumberOfRegions() + " splits.");
