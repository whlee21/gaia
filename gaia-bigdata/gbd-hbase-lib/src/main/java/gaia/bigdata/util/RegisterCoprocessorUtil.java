...
public class RegisterCoprocessorUtil {
	private static final Logger log = LoggerFactory.getLogger(RegisterCoprocessorUtil.class);
	...
	public static void run(String zkConnect, String table, String src, String dest, String className) throws IOException {
		...
		try {
			...
			ClassLoader loader = new URLClassLoader(new URL[] { fqSrcPath.toUri().toURL() },
					conf.getClass().getClassLoader(), new FsUrlStreamHandlerFactory(conf));
