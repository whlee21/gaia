...
public class CorpusGenerator extends Configured implements Tool {
	...
	public CorpusGenerator(Path input, Path output, org.apache.hadoop.mapreduce.Counters counters) {
		this.input = input;
		this.output = output;
	...
	public long generate(boolean recurse) throws IOException {
		...
		try {
			...
			PerformanceFileFilter pff = new PerformanceFileFilter(writer, key, value, conf, counters);
	...
	static class PerformanceFileFilter implements PathFilter {
		...
		public PerformanceFileFilter(SequenceFile.Writer writer, Text key, BehemothDocument value, Configuration conf,
		...
		public boolean accept(Path file) {
			try {
				...
				if (defaultIgnores.accept(file) && fs.getFileStatus(file).isDirectory() == false) {
					...
					if (unpack) {
						if (uri.endsWith(".cpio") || uri.endsWith(".jar") || uri.endsWith(".dump") || uri.endsWith(".ar")
								|| uri.endsWith("tar") || uri.endsWith(".zip") || uri.endsWith("tar.gz") || uri.endsWith(".tgz")
								|| uri.endsWith(".tbz2") || uri.endsWith(".tbz") || uri.endsWith("tar.bzip2")) {
							...
							try {
								...
								while ((entry = input.getNextEntry()) != null) {
									...
									counter++;
									if (counters != null) {
										// context.incrCounter(Counters.DOC_COUNT, 1);
										counters.findCounter(Counters.DOC_COUNT).increment(1);
									}
								}
							} catch (Throwable t) {
								if (processed == 0) {
