...
public class LanguageIdProcessor implements DocumentProcessor {
	...
	private static final Logger LOG = LoggerFactory.getLogger(LanguageIdProcessor.class);

	private Configuration config;

	private final String[] defaultLanguagesToLoad = new String[] { "af", "ar", "bg", "bn", "cs", "da", "de", "el", "en",
			"es", "et", "fa", "fi", "fr", "gu", "he", "hi", "hr", "hu", "id", "it", "ja", "kn", "ko", "lt", "lv", "mk", "ml",
			"mr", "ne", "nl", "no", "pa", "pl", "pt", "ro", "ru", "sk", "sl", "so", "sq", "sv", "sw", "ta", "te", "th", "tl",
	...
	public BehemothDocument[] process(BehemothDocument inputDoc, Context context) {
		...
		if (inputDoc.getText() == null) {
			...
			context.getCounter("LANGUAGE ID", "MISSING TEXT").increment(1);
		...
		if (inputDoc.getText().trim().isEmpty()) {
			...
			context.getCounter("LANGUAGE ID", "EMPTY TEXT").increment(1);
			return new BehemothDocument[] { inputDoc };
		}
		...
		if (context != null && lang != null)
			context.getCounter("LANGUAGE DETECTED", lang).increment(1);
