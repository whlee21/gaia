 ...
 public class LucidWorksProcessor
   implements DocumentProcessor, TikaConstants
 {
   private static final Logger LOG = LoggerFactory.getLogger(LucidWorksProcessor.class);
   ...
   public LucidWorksProcessor()
   {
     ...
     this.batchId = null;
     this.dummy = new DefaultHandler();
     this.includeImages = false;
     this.flattenCompound = false;
     this.addFailedDocs = true;
     this.addOriginalContent = false;
     this.defaultParser = TikaConfig.getDefaultConfig().getParser();
   ...
   public void setConf(Configuration conf) {
     ...
     this.batchId = conf.get("lw.batch.id");
     this.includeMetadata = conf.getBoolean("tika.metadata", true);
     this.flattenCompound = conf.getBoolean("tika.flatten", false);
     this.addFailedDocs = conf.getBoolean("tika.add.failed", true);
     this.includeImages = conf.getBoolean("tika.images", false);
     this.addOriginalContent = conf.getBoolean("tika.add.original", false);
     if ((this.defaultParser instanceof DefaultParser))
       ((DefaultParser)this.defaultParser).setFallback(ErrorParser.INSTANCE);
   }
   ...
   public BehemothDocument[] process(BehemothDocument inputDoc, Reporter reporter)
   {
     ...
     if (docMeta != null) {
       for (Map.Entry e : docMeta.entrySet()) {
         if (e.getValue() != null) {
     ...
     CollectingParser parser = new CollectingParser(this.defaultParser, this.detector, this.mimetypes, reporter, docs, metadata, this.batchId, uniqueKey, docUrl, this.includeImages, this.flattenCompound, this.addOriginalContent, this.addFailedDocs);
   ...
   private static class CollectingParser extends ParserDecorator
   {
     ...
     public CollectingParser(Parser parser, Detector detector, MimeTypes mimeTypes, Reporter reporter, List<BehemothDocument> docs, Metadata parentMeta, String batchId, String uniqueKey, String parentUrl, boolean images, boolean flatten, boolean original, boolean addFailedDocs)
     {
       super();
     ...
     public void parse(InputStream stream, ContentHandler dummy, Metadata metadata, ParseContext context)
       throws IOException, SAXException, TikaException
     {
       String resourceName = metadata.get("resourceName");
 
       StringBuilder thisUrl = new StringBuilder();
       thisUrl.append(this.parentUrl);
       StringBuilder localName = new StringBuilder();
       StringBuilder containerName = null;
       if (!this.nested.isEmpty()) {
         if (resourceName == null) {
           resourceName = new StringBuilder().append("item").append(this.count++).toString();
         }
         for (int i = 1; i < this.nested.size(); i++) {
           if (i > 1) {
             localName.append(':');
           }
           localName.append((String)this.nested.get(i));
         }
         containerName = new StringBuilder(this.parentUrl);
       ...
       if (this.nested.size() > 20) {
         LucidWorksProcessor.LOG.warn(new StringBuilder().append("Avoiding parsing loop in ").append(thisUrl).append(", nesting level: ").append(this.nested.size()).toString());
         return;
       }
       String contentType = metadata.get("Content-Type");
       if ((contentType != null) && (contentType.startsWith("image")) && (!this.images))
       {
         return;
       }
       this.nested.push(resourceName);
       try
       {
         ...
         if ((contentType == null) || (contentType.trim().length() == 0)) {
           MediaType type = this.detector.detect(stream, m);
 
           if (type != null) {
             contentType = new StringBuilder().append(type.getType()).append("/").append(type.getSubtype()).toString();
             MimeType mimetype = this.mimeTypes.forName(contentType);
             if (mimetype != null) {
         ...
         ContentHandler handler = new LucidBodyContentHandler(out, "UTF-8");
         ...
         if (archive) {
           ...
           p.parse(stream, handler, m, this.plainContext);
         else {
           super.parse(stream, handler, m, context);
         }
         ...
         if ((mime.startsWith("image")) && (!this.images))
         {
           return;
         }
         ...
         LucidWorksProcessor.LOG.trace(new StringBuilder().append("---\nExtracted content body length: ").append(body.length()).append(" extracted content body: <<").append(body).append(">>").toString());
 
         doc.setText(body);
         if (containerName != null) {
           LucidWorksProcessor.addMetadata(doc, "belongsToContainer", containerName.toString());
         }
         if (resourceName != null) {
           LucidWorksProcessor.addMetadata(doc, "resource_name", resourceName);
         }
         ...
         if ((doc.getMetadata() == null) || (!doc.getMetadata().containsKey(new Text(this.uniqueKey)))) {
           LucidWorksProcessor.setMetadata(doc, this.uniqueKey, thisUrl.toString());
         }
 
         LucidWorksProcessor.addMetadata(doc, "crawl_uri", thisUrl.toString());
 
         LucidWorksProcessor.addMetadata(doc, "batch_id", this.batchId);
 
         for (String name : m.names()) {
           String[] fieldValues = m.getValues(name);
           for (String value : fieldValues) {
             LucidWorksProcessor.addMetadata(doc, name, value);
           }
         }
 
         for (String name : this.parentMeta.names()) {
           if ((doc.getMetadata() == null) || (!doc.getMetadata().containsKey(new Text(name))))
           {
             for (String v : this.parentMeta.getValues(name))
               LucidWorksProcessor.addMetadata(doc, name, v);
           }
         }
         LucidWorksProcessor.setMetadata(doc, "parsing", "ok");
         if (this.reporter != null)
           this.reporter.getCounter("TIKA", "DOC-PARSED").increment(1L);
         this.docs.add(doc);
       }
       catch (Throwable e) {
         if (this.addFailedDocs) {
           ...
           for (String m : metadata.names()) {
             ...
             for (String v : vals) {
               LucidWorksProcessor.addMetadata(doc, m, v);
             }
           }
           StringWriter sw = new StringWriter();
           PrintWriter pw = new PrintWriter(sw);
           ...
           pw.flush();
           LucidWorksProcessor.setMetadata(doc, "parsing", new StringBuilder().append("failed: (invalid/unsupported format?) ").append(sw.toString()).toString());
           this.docs.add(doc);
         } else { LucidWorksProcessor.LOG.warn(new StringBuilder().append("Parsing ").append(thisUrl).append(" failed: ").append(e.getMessage()).toString());
           return;
         }
         if (this.reporter != null)
           this.reporter.getCounter("TIKA", "PARSING_ERROR").increment(1L);
       } finally {
         this.nested.pop();
       }
     }
