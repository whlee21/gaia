...
CLASSPATH=./conf:$(find "$PWD/target/gaia-search-0.1-dist" -name '*.jar' |xargs echo  |tr ' ' ':')

nohup java -cp $CLASSPATH -Duser.language=ko -Duser.country=KR   -Dlog4j.configuration=file:"./conf/log4j.properties" gaia.search.server.controller.GaiaSearchServer > $PWD/gaia.log 2>&1 &
