...
public class CollectionInfoService  extends BaseService {
	...
	private static org.xnap.commons.i18n.I18n i18n = I18nFactory.getI18n(CollectionInfoService.class);
	...
	public CollectionInfoService(ObjectSerializer serializer, RequestBodyParser bodyParser, String collectionName, CollectionManager cm, CoreContainer cores) {
	...
	@GET
	@Produces("text/plain")
	public Response getCollectionInfos(@Context HttpHeaders headers, @Context UriInfo ui) throws IOException, SolrServerException {
		// return handleRequest(headers, null, ui, Request.Type.GET,
		// createCollectionInfoResource(collectionName));
		Map<String, Object> collectionInfo = retrieve(null);
		return buildResponse(Response.Status.OK, collectionInfo); 
	}
	
	@GET
	@Path("{keys}")
	@Produces("text/plain")
	public Response getCollectionInfoKeys(@Context HttpHeaders headers, @Context UriInfo ui,
			@PathParam("keys") String keys) throws IOException, SolrServerException {
		// return handleRequest(headers, null, ui, Request.Type.GET,
		// createCollectionInfoResource(collectionName));
		Map<String, Object> collectionInfo = retrieve(keys);
		return buildResponse(Response.Status.OK, collectionInfo); 
	}
	
	public Map<String, Object> retrieve(String keyName) throws IOException, SolrServerException {
		if (core == null)
			throw ErrorUtils.statusExp(Response.Status.NOT_FOUND, i18n.tr("Could not find SolrCore:" + collection));
		...
		if (keyName == null) {
			...
			for (String key : reqKeys) {
				if (!VALID_KEYS.contains(key)) {
					errors.add(new Error(keyName, Error.E_MISSING_VALUE, i18n.tr("The given key does not exist:" + keyName)));
				}
			}
